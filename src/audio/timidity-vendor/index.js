/*!
 * Vendored + lightly patched copy of the `timidity` npm package
 * (https://www.npmjs.com/package/timidity) by Feross Aboukhadijeh, MIT License.
 *
 * The original package expects to be bundled with Browserify + the `brfs`
 * transform, which inlines `fs.readFileSync()` calls into actual file content
 * at build time (since there's no real filesystem in the browser). Vite uses
 * Rollup/esbuild instead, so that transform doesn't run — we replace the one
 * `fs.readFileSync(__dirname + '/freepats.cfg')` call with a Vite-native
 * `?raw` import, which achieves the exact same result (inlining the file's
 * text content as a string at build time).
 *
 * Everything else is functionally identical to the original.
 */
import LibTimidity from './libtimidity.js'
import TIMIDITY_CFG from './freepats.cfg?raw'
...
// Minimal browser-safe EventEmitter (avoids depending on Node's `events`
// built-in, which Vite externalizes and refuses to load in client code).
// Only implements the handful of methods Timidity actually uses.
class MiniEmitter {
  ...
  once(event, fn) {
    const wrapper = (...args) => { this.removeListener(event, wrapper); fn(...args) }
...
export class Timidity extends MiniEmitter {
  // `opts.audioContext` / `opts.destination`: reuse an existing shared
  // AudioContext + output node (e.g. the app's masterGain) instead of creating
  // a brand new isolated AudioContext. This matters because:
  //  - the app's volume slider controls `masterGain.gain`, not the destination
  //  - the app's chosen output device is applied via `audioContext.setSinkId()`
  // A standalone AudioContext would bypass both, so audio would always play at
  // full volume on the system default output device regardless of app settings.
  constructor (baseUrl = '/', opts = {}) {
    ...
    // Reuse the provided AudioContext if given, otherwise create our own
    // (matches original behavior as a fallback).
    this._ownsAudioContext = !opts.audioContext
    this._audioContext = opts.audioContext || new AudioContext()
    const destination = opts.destination || this._audioContext.destination
  ...
  async load (urlOrBuf) {
    ...
    if (missingCount > 0) {
      ...
      if (missingCount > 0) {
        ...
        console.warn('[timidity] Playing with missing instruments:', missingInstruments)
      }
    }
